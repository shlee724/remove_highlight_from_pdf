...
def remove_highlight_pdf_main(input_pdf, output_file_path, select_remove_algorithm):
    jpg_list = pdf_to_high_quality_jpg(input_pdf)

    if select_remove_algorithm == "gaus":
        i = 0
        for image in jpg_list:
            jpg_list[i] = remove_highlight_gaus(image)
            i += 1
    elif select_remove_algorithm == "otsu":
        i = 0
        for image in jpg_list:
            jpg_list[i] = remove_highlight_otsu(image)
            i += 1
    elif select_remove_algorithm == "extract_v":
        i = 0
        for image in jpg_list:
            jpg_list[i] = remove_highlight_extract_vchannel(image)
            i += 1        
...
def remove_highlight_extract_vchannel(img):
    # Convert pixmap to numpy array
    img_np = np.frombuffer(img.samples, dtype=np.uint8)
...
remove_highlight_pdf_main(input_pdf, output_pdf, "extract_v")